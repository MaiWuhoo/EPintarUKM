...
$leaderboard = [];
$query = $conn->prepare("SELECT p.pelajar_nama, p.pelajar_gambar, COUNT(ap.aktiviti_id) AS penyertaan
    ...
    WHERE a.persatuan_id = ? AND a.status = 'aktif' AND pm.status = 'disahkan'
    AND pm.persatuan_id = a.persatuan_id
    GROUP BY ap.pelajar_emel
    ORDER BY penyertaan DESC LIMIT 3");
